...
int threshold(values sort[], int i, info orient)
{
	...
	{
		g = 1;
		while (sort[l+g].Px == sort[l+g+1].Px  && sort[l+g].Py == sort[l+g+1].Py)
		{
			g++;
		}
		...
		for (k=0; k < g; k++)
		{
			tempz[k] = distance(orient.Xo, orient.Yo, orient.Zo, sort[l+k].Rx, sort[l+k].Ry, sort[l+k].Rz);
		}
		...
		for (t=0; t < g; t++)
		{
			if (tempz[0]+0.01 > distance(orient.Xo, orient.Yo, orient.Zo, sort[l+t].Rx, sort[l+t].Ry, sort[l+t].Rz))
			{
				fprintf(wstream, "%lf %lf %lf %d %d %d\n",
						sort[l+t].Rx, sort[l+t].Ry,
						sort[l+t].Rz, sort[l+t].IN,
						sort[l+t].Px, sort[l+t].Py);
				n++;
			}
			else{}
		}
...
void saveimage(values project[], info orient)
{
	...
	for (i = 0; i < x*y; i++)
	{
			...
			else if (v == 1)
			{
				...
				for (k=0; k < l; k++)
				{
					tempz[k] = distance(orient.Xo, orient.Yo, orient.Zo,
										project[m+k].Rx, project[m+k].Ry, project[m+k].Rz);
				}
				qsort(tempz, l ,sizeof(double), &cmp);
				
				for (k=0; k < l; k++)
				{
					if (distance(orient.Xo, orient.Yo, orient.Zo,
								 project[m+k].Rx, project[m+k].Ry, project[m+k].Rz)  == tempz[0])
						{
